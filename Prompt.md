...
│       │   └── ui/
│       │       └── PointerLockMessage.tsx # 포인터 락 실패 메시지
│       └── pages/
│           ├── Landing.tsx         # 랜딩 페이지 (/)
...
### Phase 3.5: 협동 빌딩 모드 (릴레이 레이스) ✅ 완료
**목표**: 플레이어들이 각자 구간을 만들고 연결하여 릴레이 레이스

- [x] 빌딩 페이즈 시스템 (BuildingPhase.ts)
- [x] 플레이어별 독립 빌딩 영역 할당 (50m 구간, 중앙 정렬)
- [x] 격리된 뷰 (서로 작업물 보이지 않음)
- [x] 실시간 오브젝트/마커 배치, 수정, 삭제
- [x] 속성 편집 패널 (position, rotation, scale, color)
- [x] 빌딩 타이머 (5분/10분/15분/무제한)
- [x] 테스트 플레이로 자가 검증 (Start→Finish 완주)
- [x] 검증 완료 후 읽기 전용 모드
- [x] 시간 연장 (30초) - 미검증자 있을 시
- [x] 조기 시작 - 모두 검증 완료 시
- [x] 투표 강퇴 시스템 (검증된 플레이어가 과반수 동의)
- [x] 투표 현황 실시간 표시
- [x] 강퇴된 플레이어 자동 퇴장
- [x] 랜덤 순서로 구간 연결
- [x] 릴레이 맵 데이터 생성 (체크포인트 변환)
- [x] 릴레이 레이스 플레이 (체크포인트 텔레포트)
- [x] 릴레이 영역 경계 시각화 (붉은 벽)
- [x] 빌딩 영역 물리 경계 (4면 벽 + 천장 콜라이더)
- [x] 오브젝트 배치 제한 (영역 벽 겹침 방지, 높이 제한 0-21)
- [x] 오브젝트 y=0 배치 가능 (중심점 기준으로 변경)
- [x] 일시정지 메뉴 (ESC로 열기, 방 나가기 기능)
- [x] 클릭으로 붙여넣기 (Ctrl+V 후 클릭하여 배치)
- [x] 다중 선택 색상 일괄 변경
- [x] 마커만 선택 시 색상 컨트롤 비활성화
- [x] 포인터 락 실패 피드백 (전역 적용)
- [x] Undo/Redo 히스토리 50개 제한
...
- [x] 완주 판정 및 결과 화면
- [x] 킬존 마커 (즉사 구역)
  - 원판 형태 감지 (XZ 반경 2.0, Y 높이 0.5)
  - 사망 애니메이션 (Dead) 2.5초 후 리스폰
...
- [x] **협동 빌딩 모드 (릴레이 레이스)**
  ...
  - [x] 랜덤 순서로 구간 연결
  - [x] 릴레이 맵 체크포인트 텔레포트
  - [x] 빌딩 영역 물리 경계 (벽 + 천장 콜라이더)
  - [x] 오브젝트 배치 제한 (영역 벽 겹침/높이 제한)
  - [x] 일시정지 메뉴 (ESC로 열기, 방 나가기)
  - [x] 클릭으로 붙여넣기
  - [x] 다중 선택 색상 일괄 변경
- [x] **버그 수정 및 개선**
  - [x] Socket 재연결 시 이벤트 재등록 (게임 시작 버튼 동작 안함 수정)
  - [x] 방 목록에 "게임중" 상태 방 표시
  - [x] 빌딩 모드 후 게임 시작 시 일시정지 메뉴 안 뜨게 수정
  - [x] 포인터 락 요청 에러 처리 개선
  - [x] 포인터 락 실패 시 "잠시 후 다시 클릭해주세요" 메시지 (2초간, 전역 적용)
  - [x] 빌딩 모드 y=0 위치 오브젝트 설치 가능 (중심점 기준)
  - [x] 소켓 응답에 error 필드 추가
  - [x] Undo/Redo 히스토리 50개 제한 (메모리 누수 방지)
  - [x] 마커만 선택 시 색상 컨트롤 비활성화 및 안내 메시지
