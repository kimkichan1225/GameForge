...
- **게임 모드**: Race (달리기), Shooter (FPS/TPS 슈팅)
- **멀티플레이어**: 실시간 레이스/슈터, 방 시스템, 맵 선택
...
│       │   │   ├── GunPlayer.tsx             # 슈터 플레이어
│       │   │   ├── RemoteShooterPlayer.tsx   # 리모트 슈터 플레이어
│       │   │   ├── ShooterCamera.tsx         # FPS/TPS 카메라
│       │   │   ├── ShooterHUD.tsx            # 슈터 HUD
│       │   │   ├── effects/                  # 총알 이펙트
│       │   │   │   ├── BulletEffects.tsx
│       │   │   │   ├── BulletDecal.tsx
│       │   │   │   ├── TracerLine.tsx
...
- [x] Phase 4: 총게임 모드 (Shooter) - 클라이언트 기본 구현 완료
  - [x] FPS/TPS 카메라 시스템 (V키 시점 전환)
  - [x] 무기 시스템 (라이플/샷건/스나이퍼)
  - [x] 총알 이펙트 (트레이서, 데칼, 스파크)
  - [x] 슈터 HUD (크로스헤어, 탄약, 무기 선택)
  - [x] 탄퍼짐/반동 시스템
  - [x] 재장전 시스템
  - [x] 자세별 이동 (서기/앉기/엎드리기)
  - [x] 조준 시스템 (홀드/토글, 스나이퍼 스코프)
  - [x] 팀 시스템 (개인전/팀전/점령전 대기방)
  - [x] 슈터 방 생성 (게임 옵션, 맵 필터링)
  - [x] 서버 슈터 게임 루프 (ShooterGameLoop)
  - [x] 카메라 벽 충돌/벽 관통 발사 방지
  - [ ] 서버 히트 판정 (레이캐스트)
  - [ ] 멀티플레이어 동기화 (위치/발사/피격)
  - [ ] 킬/데스/리스폰 클라이언트 연동
  - [ ] 킬 피드, 스코어보드
...
### 2026-02-13
- **슈터 벽 관통/투시 버그 수정**
  - 카메라 벽 충돌 시스템 (FPS/TPS 모두 적용)
  - 전방 벽 근접 체크로 near plane 클리핑 방지
  - 총구→조준점 2차 레이캐스트로 벽 관통 발사 방지
  - 총구가 벽 뒤에 있을 때 총구 위치 자동 조정
- **슈터 맵 제작 추후 개발 표시**
  - 방 만들기에서 Shooter 모드 시 "맵 제작 & 플레이" 비활성화
  - "추후 개발" 라벨 표시

### 2026-02-12
- **슈터 팀 시스템**
  - 팀전/점령전 대기방에서 팀 선택 (Team A / Team B)
- **맵 브라우저 필터링 수정**
  - 전체 맵 탐색 시 필터링된 맵만 보이던 버그 수정
- **슈터 방 생성**
  - 게임 방식 선택 (개인전/팀전/점령전)
  - 킬 제한, 시간 제한, 시점(FPS/TPS) 옵션
  - 슈터 모드 맵 필터링
